...
class WoopTokenService {
  ...
  bool _isValidAddress(String address) {
    if (address.isEmpty) {
      Get.snackbar(
        'Error',
        'Por favor ingresa una dirección BSC',
    ...
    if (address.length != 42) {
      Get.snackbar(
        'Error',
        'La dirección debe tener 40 caracteres (sin contar 0x)',
        snackPosition: SnackPosition.BOTTOM,
        backgroundColor: const Color(0xFFFF3B30),
        colorText: Colors.white,
      );
    ...
    if (!RegExp(r'^[0-9a-fA-F]+$').hasMatch(hexPart)) {
      Get.snackbar(
        'Error',
        'La dirección contiene caracteres inválidos. Solo se permiten números y letras de A a F',
        snackPosition: SnackPosition.BOTTOM,
        backgroundColor: const Color(0xFFFF3B30),
        colorText: Colors.white,
      );
